...
struct MyPickerView: View {
    ...
    var body: some View {
        MyPicker(numberOfSegments: 4, highlightedHandle: Segment(color: .yellow))
    }
}

struct MyPicker<Content: View >: View {
    @State private var index: Int = 0
    @State private var offset: CGFloat = 0
    @State private var isGestureActive: Bool = false
    let numberOfSegments: Int
    var highlightedHandle: Content
    var body: some View {
                    ...
                    .gesture(DragGesture(minimumDistance: 0 , coordinateSpace: .global)
                        ...
                        .onEnded({ value in
                            if abs(value.translation.width) >= segmentWidth / 2 {
                                ...
                                self.index = nextIndex.keepIndexInRange(min: 0, max: self.numberOfSegments - 1)
...
struct ContentView_Previews: PreviewProvider {
    static var previews: some View {
        MyPickerView(numberOfSegments: 4)
    }
}
